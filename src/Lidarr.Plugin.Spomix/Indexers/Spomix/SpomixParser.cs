...
{
    public class SpomixParser : IParseIndexerResponse
    {
        private static readonly int[] _bitrates = new[] { 1, 3, 9 };

        public SpomixUser User { get; set; }

        public IList<ReleaseInfo> ParseResponse(IndexerResponse response)
        {
            var torrentInfos = new List<ReleaseInfo>();

            var jsonResponse = new HttpResponse<SpomixSearchResponse>(response.HttpResponse);
        ...
        private static ReleaseInfo ToReleaseInfo(SpomixGwAlbum x, int bitrate)
        {
            ...
            {
                Guid = $"Spomix-{x.AlbumId}-{bitrate}",
                Artist = x.ArtistName,
                Album = x.AlbumTitle,
                DownloadUrl = x.Link,
                InfoUrl = x.Link,
                PublishDate = publishDate,
                DownloadProtocol = nameof(SpomixDownloadProtocol)
            };
