...
{
    public class Spomix : DownloadClientBase<SpomixSettings>
    {
        private readonly ISpomixProxy _proxy;

        public Spomix(ISpomixProxy proxy,
        ...
        private ValidationFailure TestSettings()
        {
            ...
            {
                return new NzbDroneValidationFailure(string.Empty, "Spomix must have 'Create Album Folders' enabled")
                {
                    InfoLink = HttpRequestBuilder.BuildBaseUrl(Settings.UseSsl, Settings.Host, Settings.Port, Settings.UrlBase),
                    DetailedDescription = "Spomix must have 'Create Album Folders' enabled, otherwise Lidarr will not be able to import the downloads",
                };
            }

            if (!config.CreateSingleFolder)
            {
                return new NzbDroneValidationFailure(string.Empty, "Spomix must have 'Create folder structure for singles' enabled")
                {
                    InfoLink = HttpRequestBuilder.BuildBaseUrl(Settings.UseSsl, Settings.Host, Settings.Port, Settings.UrlBase),
                    DetailedDescription = "Spomix must have 'Create folder structure for singles' enabled, otherwise Lidarr will not be able to import single downloads",
                };
            }
            ...
            {
                return new NzbDroneValidationFailure(string.Empty, "Could not login to Spomix. Invalid ARL?")
                {
                    InfoLink = HttpRequestBuilder.BuildBaseUrl(Settings.UseSsl, Settings.Host, Settings.Port, Settings.UrlBase),
                    DetailedDescription = "Spomix requires a valid ARL to initiate downloads",
                };
            }
